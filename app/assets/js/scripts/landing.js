    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    'Aucune version de<br>Java présente',
                    'Pour utiliser le Launcher, il vous faut la version 16 ou + de Java. Voulez-vous essayer de la télécharger ?',
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            ...
                            'Une installation de Java 16 ou + est nécessaire.<br><br>Vous pouvez trouver de l\'aide sur le Discord Académie.',
                            'D\'accord',
                            'Retour'
                        )
                        setOverlayHandler(() => {
...
function dlAsync(login = true){
    ...
    if(login) {
        if(ConfigManager.getSelectedAccount() == null){
            loggerLanding.error('Vous devez être identifié.')
            return
        }
    }

    setLaunchDetails('Merci de Patientier...')
    toggleLaunchArea(true)
    setLaunchPercentage(0, 100)
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur lors du Lancement', err.message || 'Pour connaître les détails, (CTRL + Shift + i)..')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('Erreur lors du Lancement', 'Pour connaître les détails, (CTRL + Shift + i)..')
        }
    })
        ...
        if(m.context === 'validate'){
            switch(m.data){
                ...
                case 'files':
                    ...
                    loggerLaunchSuite.log('Vérification des fichiers.')
                ...
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            ...
                            'Impossible de se connecter au serveur de fichiers...'
                        )
                    } else {
                        showLaunchFailure(
                            'Erreur de Téléchargement',
                            'Pour connaître les détails, (CTRL + Shift + i).'
                        )
                    }
            ...
            if(m.result.forgeData == null || m.result.versionData == null){
                ...
                loggerLaunchSuite.error('Error lors du Lancement', m.result.error)
                showLaunchFailure('Erreur lors du Lancement', 'Pour connaître les détails, (CTRL + Shift + i).')
            ...
            if(login && allGood) {
                ...
                const gameErrorListener = function(data){
                    data = data.trim()
                    if(data.indexOf('Impossible de charger ou de trouver net.minecraft.launchwrapper.Launch') > -1){
                        loggerLaunchSuite.error('Impossible de lancer le jeu, LaunchWrapper n\'est pas présent.')
                        showLaunchFailure('Erreur lors du Lancement', 'Le fichier, LaunchWrapper, est impossible d\'accès. En conclusion, le jeu ne peux pas se lancer.<br><br>Pour corriger ceci, vous pouvez tenter de désactiver votre antivirus et de relancer le Launcher.')
                    }
                }
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        loggerLaunchSuite.log('Erreur pendant la récupération du Distribution Index.', err)
        refreshDistributionIndex(false, (data) => {
            onDistroRefresh(data)
            serv = data.getServer(ConfigManager.getSelectedServer())
            aEx.send({task: 'execute', function: 'validateEverything', argsArr: [ConfigManager.getSelectedServer(), DistroManager.isDevMode()]})
        }, (err) => {
            loggerLaunchSuite.error('Impossible de rafraichir le Distribution Index.', err)
            if(DistroManager.getDistribution() == null){
                showLaunchFailure('Erreur Fatal', 'Impossible de charger un Distribution Index Local. Pour connaître les détails, (CTRL + Shift + i).')
...
function setNewsLoading(val){
    if(val){
        const nLStr = 'Chargement en cours...'
        let dotStr = '..'
