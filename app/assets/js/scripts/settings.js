            ...
            AuthManager.addMicrosoftAccount(authCode).then(value => {
                ...
                .catch((displayableError) => {
                    ...
                    if(isDisplayableError(displayableError)) {
                        ...
                        // Uh oh.
                        msftLoginLogger.error('Erreur inconnu lors de l\'authentification', displayableError)
                        actualDisplayableError = {
                            title: 'Erreur inconnu lors de l\'authentification.',
                            desc: 'Une erreur inconnue est survenue lors de l\'authentification. Veuillez réessayer.'
                        }
                    }
        ...
        val.onclick = (e) => {
            ...
            for(let i=0; i<selectBtns.length; i++){
                if(selectBtns[i].hasAttribute('selected')){
                    ...
                    selectBtns[i].innerHTML = 'Sélectionner Account'
                }
            }
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Attention<br>C\'est le dernier compte!',
                    'Vous êtes sur le point de supprimer le dernier compte. Vous ne pourrez plus vous connecter à Minecraft.',
                    'Je suis sur de vouloir supprimer ce compte.',
                    'Annuler'
                )
                setOverlayHandler(() => {
    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            if(arguments_.length > 1 && arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
                // User cancelled.
                msftLogoutLogger.info('Déconnexion annulé par le client.')
                return
            }
        ...
        msftLogoutLogger.info('Déco. réussi. uuid:', uuid)
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = 'Channel BETA'
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        settingsUpdateTitle.innerHTML = 'Vous avez déjà la dernière version !'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Vérifier les MaJ...', false, () => {
            if(!isDev){
